...
func (server *Server) listAccount(ctx *gin.Context) {
	...
	arg := db.ListAccountParams{Limit: req.Paging.Limit, Offset: req.Paging.Offset}
...
func (server *Server) listAccountByOwner(ctx *gin.Context) {
	...
	authPayload := ctx.MustGet(authorizationPayloadKey).(*token.Payload)
	arg := db.ListAccountByOwnerParams{
		Limit:  req.Paging.Limit,
		Offset: req.Paging.Offset,
		Owner:  authPayload.Username,
	}

	accounts, err := server.store.ListAccountByOwner(ctx, arg)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, errorResponse(err))
		return
	}
